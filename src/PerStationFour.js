...
import StationFourIncident from './StationFourIncident';
import StationFourEmergency from './StationFourEmergency'; // Import StationFourEmergency compFournt

const PerStationFour = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const q = query(collection(db, 'anonymous-reports'), where('location', '==', '4'));
  ...
  return (
    <div>
      <h1>BUTUAN CITY POLICE STATION 4</h1>
     
      <button onClick={handleShowIncident} style={{ color: 'black', backgroundColor: incidentButtonClicked ? 'blue' : 'white', padding: '10px 20px', border: 'nFour', borderRadius: '5px', marginBottom: '20px', fontSize: 'medium', margin: '0 10px' }}>Show Incident Reports</button>
      <button onClick={handleShowAnonymousReports} style={{ color: 'black', backgroundColor: anonymousButtonClicked ? 'blue' : 'white', padding: '10px 20px', border: 'nFour', borderRadius: '5px', marginBottom: '20px', fontSize: 'medium', margin: '0 10px' }}>Show Anonymous Reports</button>
      <button onClick={handleShowEmergency} style={{ color: 'black', backgroundColor: emergencyButtonClicked ? 'blue' : 'white', padding: '10px 20px', border: 'nFour', borderRadius: '5px', marginBottom: '20px', fontSize: 'medium', margin: '0 10px' }}>Show Emergency Reports</button>
      ...
      {showIncident && <StationFourIncident />}
      {showEmergency && <StationFourEmergency />} {/* Display StationFourEmergency compFournt */}
    </div>
  );
};

export default PerStationFour;
